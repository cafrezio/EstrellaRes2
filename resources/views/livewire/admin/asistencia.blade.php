<div>
    ...
    @if(count($eventos) == 0)
    <div class="card-body">
        <h5>No tiene Eventos activos asignados.</h5>
    </div>
    @else
            ...
            <div class="row">
                ...
                <div class="col">
                    ...
                    <select class="form-control" name="funcion" wire:model="funcionSel">
                        ...
                        <option value="{{ $funcion->func_id }}">{{ $funcion->titulo }} - {{ utf8_encode(strftime("%a
                            %d/%m", strtotime($funcion->fecha))) }} - {{ strftime("%H:%M", strtotime($funcion->horario
                            ))}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-1">
                    ...
                    <select class="form-control" name="funcion" wire:change="changeColor($event.target.value)">
                        ...
                        <option value="{{ $color->id }}" {{ ($colorSel===$color->id)?'selected' : '' }}>{{ $color->color
                            }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col">
                    <span class="badge badge-secondary" style="width:100%">
                        <h5>Reservas: <b> {{ $totRes }} </b> / Asistencia: <b> {{ $totAsist }} </h5>
                    </span>
                    <div class="progress" style="height:30px; margin-top:5px">
                        @if ($totRes > 0)
                        <div class="progress-bar bg-success" style="width:{{ sprintf(" %.0f%%", $totAsist/$totRes * 100)
                            }} ;height:30px">{{ sprintf("%.0f%%", $totAsist/$totRes * 100) }}</div>
                        @endif
                    </div>
                </div>
                ...
                <div class="col">
                    <span style="text-align: center">
                        <h4>Total Función: <b> {{ '$ ' . number_format($totIng) }} </b></h4>
                    </span>
                    <span style="text-align: center">
                        <h5>Total Evento: {{ '$ ' . number_format($totEvento) }} </h5>
                    </span>
                    <span style="text-align: center">
                        <h5>Asist: {{number_format($totAsistEvento) }} | Prom:
                            @if($totAsistEvento == 0)
                            $ 0
                            @else
    ...
    <div class="card-body">
        <table class="table table-striped">
            ...
            <tr class={{ $classrow }}>
                ...
                <td>
                    <span class="badge"
                        style="background-color: {{ $codColorSel }}; font-size: 100%; border: 1px solid gray">
                        {{ $reserva->usuario }}
                    </span>
                ...
                <td>
                    ...
                    {{ $reserva->cant_adul }}
                    @else
                    <select class="form-control rowh" name="cant_adul"
                        wire:change="changeCantAdul({{ $reserva->id }}, $event.target.value)">
                        @for ($i=1; $i <= 10; $i++ ) <option value="{{ $i }}" {{ ($i===$reserva->cant_adul)? 'selected'
                            : '' }}>{{ $i }}</option>
                            @endfor
                    </select>
                    @endif
                ...
                <td>
                    ...
                    {{ $reserva->cant_esp }}
                    @else
                    <select class="form-control rowh" name="cant_esp"
                        wire:change="changeCantEsp({{ $reserva->id }}, $event.target.value)">
                        @for ($i=0; $i <= 10; $i++ ) <option value="{{ $i }}" {{ ($i===$reserva->cant_esp)? 'selected' :
                            '' }}>{{ $i }}</option>
                            @endfor
                    </select>
                    @endif
                ...
                <td>
                    ...
                    <span class="badge"
                        style="background-color: {{ $funcion->codigo_color }}; font-size: 100%; border: 1px solid white">
                        {{ $funcion->titulo }} - {{ utf8_encode(strftime("%a %d/%m", strtotime($funcion->fecha))) }} -
                        {{ strftime("%H:%M", strtotime($funcion->horario ))}}
                    </span>
                    @endif
                    @endforeach
                    @else
                    <select class="form-control rowh" name="funcion2"
                        wire:change="changeFunc2({{ $reserva->id }}, $event.target.value)">
                ...
                <td>
                    @if ($reserva->asist == 1)
                    <input type="checkbox" {{ ($reserva->cancel === 1)? 'checked' : '' }} wire:change="changeCancel({{
                    $reserva->id }}, $event.target.checked)">
                    @endif
                </td>
            ...
            <tr>
                ...
                <td>
                    <select class="form-control" name="cant_adul" wire:change="changeNewCantAdul($event.target.value)">
                        @for ($i=0; $i <= 10; $i++ ) <option value="{{ $i }}" {{ ($i==$newCantAdul)? 'selected' : '' }}>
                            {{ $i }}</option>
                            @endfor
                    </select>
                </td>
                <td>
                    <select class="form-control" name="cant_esp" wire:change="changeNewCantEsp($event.target.value)">
                        @for ($i=0; $i <= 10; $i++ ) <option value="{{ $i }}" {{ ($i==$newCantEsp)? 'selected' : '' }}>
                            {{ $i }}</option>
                            @endfor
                    </select>
                </td>
                ...
                <td>
                    <select class="form-control" name="newfuncion2" wire:change="changeNewFun2($event.target.value)">
                        ...
                        <option value="{{ $funcion->func_id }}">
                            {{ $funcion->titulo }} - {{ utf8_encode(strftime("%A %d de %B", strtotime($funcion->fecha)))
                            }} - {{ strftime("%H:%M", strtotime($funcion->horario ))}}
                        </option>
                        @endif
                        @endforeach
